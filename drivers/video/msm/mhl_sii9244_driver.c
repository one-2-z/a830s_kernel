...
void rcp_cbus_uevent(u8 rcpCode)	
{
	...
	printk("[SKY_MHL]%s : RCP Message Recvd , rcpCode = 0x%x\n",__func__,rcpCode);
		...
		*/
		...
		case 0x44:  case 0x60: key = GENERIC_PLAY_PAUSE  ; break;// 207, java->MEDIA_PLAY_PAUSE
		case 0x45:  case 0x64: key = GENERIC_STOP ; break;// STOP-key  //KEY_STOPCD
		case 0x46:  case 0x61: key = GENERIC_PLAY_PAUSE  ; break;// 119, java->MEDIA_PLAY_PAUSE
		case 0x48:  key = GENERIC_PREVIOUS  ; break;// <<  FORWARD로 동작 되어야 함..
		case 0x49:  key = GENERIC_NEXT  ; break;// >>	  REWIND로 동작 되어야 함. 	
		case 0x4B:  key = GENERIC_FAST_FORWARD ; break;// <<ㅇ
		case 0x4C:  key = GENERIC_REWIND  ; break;// >>
...
void MHL_En_Control(bool on)
{
	...
	printk("[SKY_MHL] MHL_En_Control  : %d\n", on);
	 ...
	 init_done = FALSE;
#endif		
//	cancel_delayed_work(&f_mhl_cable_check_control.work); // 20111212, kkcho, for stop polling-check
   	}	   	
}
...
void process_mhl_discovery_start(void)
{
	...
	if((int4Status & MHL_EST_INTR4)&&(mhl_status_value.cbus_connected))
	{
		...
		if(rsen_timer == FALSE)  // kkcho_1118
		{
			rsen_timer = TRUE;
