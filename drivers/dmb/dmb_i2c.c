...
SIDE EFFECTS
======================================================================*/
uint8 dmb_i2c_write_data(uint16 reg, uint8 *data, uint16 width)
{
  static int ret = 0;
  struct i2c_msg msgs[] = 
  {
    {
      .addr = dmb_i2c_client->addr,  .flags = 0, .len = width,  .buf = data,
    },
  };
...
SIDE EFFECTS
======================================================================*/
uint8 dmb_i2c_write_len(uint8 chip_id, unsigned int uiAddr, uint8 width, uint8 *data, uint16 data_len)
{
  static int ret = 0;
  ...
  memcpy(&buf[width], data, data_len); //temp

  DMB_MSG_I2C_DBG("[%s] ID[0x%02x] reg[0x%04x] cnt[%d]\n", __func__, dmb_i2c_client->addr, uiAddr, data_len);

  ret = dmb_i2c_write_data(uiAddr, buf, reg_len);
...
//EF33S QUP I2C 사용시 두번 보내야함
  ...
  if(ret < 0)
  {
    DMB_MSG_I2C_DBG("[%s] read error!!![%d]\n", __func__, ret);
    return FALSE;
  }
  else
  {
...
SIDE EFFECTS
======================================================================*/
void dmb_i2c_read_len(uint8 chip_id, unsigned int uiAddr, uint8 ucRecvBuf[], uint16 ucCount, uint8 width)
{
  static int ret = 0;
  unsigned char reg_len = 0;
  unsigned char buf[2];
  ...
  DMB_MSG_I2C_DBG("[%s] ID[0x%02x] reg[0x%04x] cnt[%d]\n", __func__, dmb_i2c_client->addr, uiAddr, ucCount);

  ret = dmb_i2c_read_data(uiAddr, reg_len, buf, ucRecvBuf, ucCount);
  ...
  {    
    ...
    if(ucCount == 2)
    {
      ...
      DMB_MSG_I2C_DBG("R2 [%x] read [%x][%x]\n", buf[0],ucRecvBuf[0],ucRecvBuf[1]);
    }
    else if(ucCount ==4)
    {
      //uiData = (ucRecvBuf[0] << 8) | ucRecvBuf[1];
      DMB_MSG_I2C_DBG("R4 [%x]read [%x][%x][%x][%x]\n", buf[0],ucRecvBuf[0],ucRecvBuf[1],ucRecvBuf[2],ucRecvBuf[3]);
    }
