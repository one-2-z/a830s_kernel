...
int sharp_bb_get_ant_value(int sqindicator)
{
  ...
  int level = 0;
  int ant_table[] = {80, 50, 30};

  if(sqindicator >= ant_table[0]) level = 3;
  else if((sqindicator >= ant_table[1] && sqindicator < ant_table[0])) level = 2;
  else if((sqindicator >= ant_table[2] && sqindicator < ant_table[1])) level = 1;
  else if(sqindicator < ant_table[2]) level = 0;

  if((level > 0) && (abs(level - saved_level) == 1))
  {
    if(sqindicator < (ant_table[level-1] - 10))
    {
      saved_level = level;
    }
  }
  ...
  // apply Alpha verion App(Pixela) (ant bar 0, 1(>20), 2(>40), 3(>60))
  return saved_level;
}
