...
{
#ifdef FEATURE_DMB_I2C_CMD
  dmb_i2c_read_len(chipid, uiAddr, ucRecvBuf, (uint16)ucCount, sizeof(uint8));
#endif /* FEATURE_DMB_I2C_CMD */
...
uint8 tcc3170_init(void)
{
  ...
  tcbd_init_parser(0, (tcbd_stream_callback)tcc3170_parse_callback);
#elif defined(FEATURE_DMB_SPI_IF)
...
int tcc3170_read_fib(uint8 *fibs)
{
  ...
  ret = tcbd_read_fic_data(dev, fic_data.fic_buf, wFicLen);
  ...
  fic_data.fib_num = wFicLen;

#ifdef DEBUG_DUMP_FILE
	//file_dump(ts_data.fic_buf, ts_data.fic_size, "/mnt/sdcard/Movies/tcc3170.bin");
	file_dump(fic_data.fic_buf, fic_data.fib_num, "/data/local/tmp/tcc3170.bin");
#endif
...
/*====================================================================
FUNCTION       tcc3170_parse_callback
DESCRIPTION 
DEPENDENCIES
RETURN VALUE
SIDE EFFECTS
======================================================================*/
void tcc3170_parse_callback(int dev_idx, unsigned char *_stream, int _size, int _subch_id, int _type)
{
  ...
  {
  case 0: // MSC
    if (_size > BB_MAX_DATA_SIZE)
    {
      TDMB_MSG_TCC_BB("[%s] MSC size overflow [%d]!!\n", __func__, _size);
      _size = BB_MAX_DATA_SIZE;
    }
    
    memcpy(ts_data.msc_buf, _stream, _size);
    ts_data.msc_size = _size;
    //tdmb_send_sig();
    break;
  case 1: // FIC
    ...
    memcpy(ts_data.fic_buf, _stream, _size);
    ts_data.fic_size = _size;
    break;
  case 2: // STATUS
...
/*====================================================================
FUNCTION       tcc3170_parse_data
DESCRIPTION 
DEPENDENCIES
RETURN VALUE
SIDE EFFECTS
======================================================================*/
void tcc3170_parse_data(uint8* data_buf, uint32 size)
{
  //TDMB_MSG_TCC_BB("[%s] \n", __func__);
...
/*====================================================================
FUNCTION       tcc3170_put_data
DESCRIPTION 
DEPENDENCIES
RETURN VALUE
SIDE EFFECTS
======================================================================*/
int tcc3170_put_data(uint8* data_buf, uint32 size, uint8 type)
{
  ...
  {
  case 0: /* MSC */
    tcc3170_put_msc_data(data_buf, size);
    break;

  case 1: /* FIC */
    tcc3170_put_fic_data(data_buf, size);
    break;
...
SIDE EFFECTS
======================================================================*/
int tcc3170_put_fic_data(uint8* data_buf, uint32 size)
{
  //TDMB_MSG_TCC_BB("[%s]\n", __func__);
...
SIDE EFFECTS
======================================================================*/
int tcc3170_put_msc_data(uint8* data_buf, uint32 size)
{
