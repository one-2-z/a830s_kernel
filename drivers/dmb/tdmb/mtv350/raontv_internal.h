...
extern BOOL g_fRtvStreamEnabled;
	...
	extern U8 g_bRtvIntrMaskRegL; //0928 build error
   #endif
#endif	
 ...
 *============================================================================*/ 
	...
	extern BOOL g_fRtvTdmbConfiguredFIC;
	...
	extern BOOL g_fTdmbPlayState;
     #endif
  #endif	
#endif

/* Forward prototype. */
static INLINE void rtv_SetupMemory_MSC1(E_RTV_TV_MODE_TYPE eTvMode, U16 wThresholdSize);
...
{	
	...
	switch( eTvMode )
	{
		...
		case RTV_TV_MODE_1SEG:
			...
			RTV_REG_SET(0x62, g_bRtvIntrMaskRegL|MSC0_INTR_BITS|MSC1_INTR_BITS); // FIC, MSC0, MSC1
	  #endif
			break;
	  ...
	  #if defined(RTV_IF_SPI) || defined(RTV_IF_EBI2)
			RTV_REG_MAP_SEL(HOST_PAGE);
		    	RTV_REG_SET(0x62, g_bRtvIntrMaskRegL|MSC0_INTR_BITS|MSC1_INTR_BITS|FIC_E_INT); // FIC, MSC0, MSC1
		    	...
		    	RTV_REG_MAP_SEL(HOST_PAGE);
		    	RTV_REG_SET(0x62, g_bRtvIntrMaskRegL|FIC_E_INT); // FIC
		 #endif
			   ...
			   if(g_fTdmbPlayState == FALSE) // I2C read mode
			   {
				   ...
				   rtv_ResetMemory_FIC(); /* Disable FIC. */
			   }
			   else
			   {
				   RTV_REG_MAP_SEL(HOST_PAGE);
				   ...
				   RTV_REG_MAP_SEL(HOST_PAGE); // Restore
				   RTV_REG_SET(0x29, 0x00);
			   }			   	
		#endif
	#endif		 
			/* Do not disable MSC memory. MSC memory already disabled. */
			break;
#endif			
...
static INLINE void rtv_StreamRestore(E_RTV_TV_MODE_TYPE eTvMode)
{
	...
	switch( eTvMode )
	{
		...
		case RTV_TV_MODE_1SEG:
			...
			RTV_REG_MAP_SEL(HOST_PAGE);
			RTV_REG_SET(0x62, g_bRtvIntrMaskRegL); // MSC1
	#endif
			break;
  #endif
		...
		case RTV_TV_MODE_TDMB:
		    ...
		    if(g_fRtvTdmbConfiguredFIC == TRUE)	    
				rtv_SetupMemory_FIC(); // Restore FIC memory.		    
			...
			RTV_REG_MAP_SEL(HOST_PAGE);
		   	RTV_REG_SET(0x62, g_bRtvIntrMaskRegL); // FIC
		     if(g_fRtvTdmbConfiguredFIC == TRUE)	   
			   rtv_SetupMemory_FIC(); // Restore FIC memory.		   
		   #endif
			...
			else
			{
			 ...
			 /* Do NOT enable MSC memory! MSC memory was enable when the sub-channel was opened. */
			 if(g_fRtvTdmbConfiguredFIC == TRUE)	    
			      rtv_SetupMemory_FIC(); // Restore FIC memory.		    
              #endif
        #endif
...
static INLINE void rtv_StreamEnable(void)
{
	...
	g_bRtvIntrMaskRegL = 0x8F; //[6] MSC1 over-run interrupt [5] MSC1 under-run interrupt [4] MSC1 interrupt
    	RTV_REG_SET(0x62, g_bRtvIntrMaskRegL); 

	rtv_SetupMemory_MSC1(RTV_TV_MODE_1SEG, g_nRtvMscThresholdSize); // Restore MSC1 memory.
	    
  #elif defined(RTV_IF_MPEG2_SERIAL_TSIF) || defined(RTV_IF_SPI_SLAVE) || defined(RTV_IF_QUALCOMM_TSIF)	
	RTV_REG_MAP_SEL(HOST_PAGE);
	RTV_REG_SET(0x29, 0x00);	 
	
	rtv_SetupMemory_MSC1(RTV_TV_MODE_1SEG, RTV_TS_PACKET_SIZE); // Restore MSC1 memory.
  #endif	 
...
#if (defined(RTV_TDMB_ENABLE)||defined(RTV_DAB_ENABLE)) && (defined(RTV_ISDBT_ENABLE)||defined(RTV_FM_ENABLE))
