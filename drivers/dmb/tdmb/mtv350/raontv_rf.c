...
#ifdef RTV_DAB_ENABLE
		case RTV_TV_MODE_DAB_B3 : case RTV_TV_MODE_DAB_L :  case RTV_TV_MODE_TDMB:  REG54 = 0x08; break;
#endif
		default: return;
	}
...
INT rtvRF_ConfigureAdcClock(E_RTV_TV_MODE_TYPE eTvMode, E_RTV_ADC_CLK_FREQ_TYPE eAdcClkFreqType)
{
	...
	{
	     TDMB_MSG_RTV_BB("[rtvRF_ConfigureAdcClock] Unsupport ADC clock type: %d\n", eAdcClkFreqType);
	...
	switch( eTvMode )
	{
		...
		case RTV_TV_MODE_DAB_L : REGE9 = (0x54 & 0xF0); break;
#endif
	...
	for(i=0; i<10;i++)
	{
		...
		{
			TDMB_MSG_RTV_BB("[rtvRF_ConfigureAdcClock] CLOCK SYNTH 1st step  UnLock..\n");
		}
...
{	
	...
	if(pbAdcClkSynTbl[0] == 0xFF)
    {
        TDMB_MSG_RTV_BB("[rtvRF_ChangeAdcClock] Unsupport ADC clock type: %d\n", eAdcClkFreqType);
		return RTV_UNSUPPORT_ADC_CLK;
    }
...
#ifdef RTV_DAB_ENABLE
		case RTV_TV_MODE_DAB_B3 : case RTV_TV_MODE_DAB_L : case RTV_TV_MODE_TDMB:  REGE9 = (0x54 & 0xF0); break;
#endif
		default: return RTV_INVAILD_TV_MODE;
	}
	...
	for(i=0; i<10;i++)
	{
		...
		{
			TDMB_MSG_RTV_BB("[rtvRF_ChangeAdcClock] CLOCK SYNTH 1st step  UnLock..\n");
		}
	...
	{
		TDMB_MSG_RTV_BB("[rtvRF_ChangeAdcClock] CLOCK SYNTH UnLock..\n");
	}
#endif
...
//TDMB_MSG_RTV_BB("[rtvRF_SetFrequency] Para verify  dwChFreqKHz = %d  nChNum = %d  eTvMode = %d\n",dwChFreqKHz,nChNum,eTvMode);
	...
	switch( eTvMode )
	{
		...
		case RTV_TV_MODE_DAB_B3 :			
			if(g_curDabSetType != RTV_TV_MODE_DAB_B3)
			{
				...
				nNumTblEntry = sizeof(t_BAND3_INIT) / sizeof(RTV_REG_INIT_INFO);
		...
		case RTV_TV_MODE_1SEG :			
			...
			do{
				...
				{ 
					RTV_REG_SET(0x20, nChNum);	
					RTV_DELAY_MS(2);	//2ms Delay 
				//	TDMB_MSG_RTV_BB("[rtvRF_SetFrequency] 1SEG PLL verify Re-Try!!!  PLLNF_I2C = 0x%0x  RD_PLLNF = 0x%0x RD_PLLNF*2 = 0x%0x\n",wr_dwPllNF_Verify_val,rd_dwPllNF_Verify_val,rd_dwPllNF_Verify_val>>1);
				}
		...
		case RTV_TV_MODE_FM :		
			...
			do{
				...
				{ 
					RTV_REG_SET(0x20, 0x00);	
					RTV_DELAY_MS(2);	//2ms Delay 
				//	TDMB_MSG_RTV_BB("[rtvRF_SetFrequency] FM PLL verify Re-Try!!!  PLLNF_I2C = 0x%0x  RD_PLLNF = 0x%0x RD_PLLNF*2 = 0x%0x\n",wr_dwPllNF_Verify_val,rd_dwPllNF_Verify_val,rd_dwPllNF_Verify_val>>1);
				}
			}while(PLL_Verify_cnt--);
		...
		case RTV_TV_MODE_TDMB :	
			...
			do{
				...
				{ 
					RTV_REG_SET(0x20, 0x00);	
					RTV_DELAY_MS(2);	//2ms Delay 
			//		TDMB_MSG_RTV_BB("[rtvRF_SetFrequency] TDMB PLL verify Re-Try!!!  PLLNF_I2C = 0x%0x  RD_PLLNF = 0x%0x RD_PLLNF*2 = 0x%0x\n",wr_dwPllNF_Verify_val,rd_dwPllNF_Verify_val,rd_dwPllNF_Verify_val>>1);
				}
			}while(PLL_Verify_cnt--);
		...
		case RTV_TV_MODE_DAB_L :
			...
			do{
				...
				{ 
					RTV_REG_SET(0x20, 0x00);	
					RTV_DELAY_MS(2);	//2ms Delay 
				//	TDMB_MSG_RTV_BB("[rtvRF_SetFrequency] DAB PLL verify Re-Try!!!  PLLNF_I2C = 0x%0x  RD_PLLNF = 0x%0x RD_PLLNF*2 = 0x%0x\n",wr_dwPllNF_Verify_val,rd_dwPllNF_Verify_val,rd_dwPllNF_Verify_val>>1);
				}
			}while(PLL_Verify_cnt--);
			RTV_REG_MAP_SEL(0x06); // ofdm
			RTV_REG_SET(0x10,0x48); 
...
INT rtvRF_Initilize(E_RTV_TV_MODE_TYPE eTvMode)
{
	...
	switch( eTvMode )
	{
		...
		case RTV_TV_MODE_DAB_B3:
			...
			nNumTblEntry = sizeof(t_DAB_INIT) / sizeof(RTV_REG_INIT_INFO);
...
#ifdef RTV_PWR_DCDC
	{
		...
		{
			TDMB_MSG_RTV_BB("[rtvRF_Initilize] Clock Unlock \n");
		}
	}
#endif
//////////////////////////////CODE PATCH FOR OTP////////////////////////////////////
