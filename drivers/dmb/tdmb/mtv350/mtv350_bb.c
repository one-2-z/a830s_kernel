...
void mtv350_ch_scan_start(int freq, int band, unsigned char for_air)
{
  ...
  rtvTDMB_SetPlayState(FALSE);

  res = rtvTDMB_ScanFrequency(freq);

  TDMB_MSG_RTV_BB("[%s] Channel Frequency[%d] Band[%d] Mode[%d] Res[%d] \n", __func__, freq, band, for_air, res);
...
{ 
  ...
  uiServiceType = g_stEnsembleInfo.astSubChInfo[0].uiServiceType;
  ...
  rtvTDMB_SetPlayState(TRUE);

#ifdef FEATURE_TDMB_MULTI_CHANNEL_ENABLE// PJSIN 20110223 add-- [ 1 
  rtvTDMB_OpenFIC();
#endif// ]-- end 
 
  res = rtvTDMB_OpenSubChannel(uiSubChID, uiServiceType, 188*8);
...
int mtv350_read_fib(uint8 *fibs)
{
  ...
  //return rtvTDMB_GetFIB(fibs);
  return rtvTDMB_ReadFIC(fibs);
}
...
static uint16 mtv350_Ant_Level(uint16 pcber)
{
  ...
  uint16 ant_tbl[] = {801, // 0 <-> 1
                               501, // 1 <-> 2
                               301, // 2 <-> 3
                               151, // 3 <-> 4
                               101,  // 4 <-> 5
                               1}; // 5 <-> 6
  uint16 hystery_value[]= {80,15};

  //TDMB_MSG_RTV_BB("[%s]\n", __func__);

  if(pcber < ant_tbl[5]) level = 6;//(ber == 0 ) printf("ANT: 6\n");
  else if(pcber >= ant_tbl[5] && pcber < ant_tbl[4]) level = 5;//(ber >= 1 && ber <= 100) printf("ANT: 5\n");
...
void mtv350_get_ber(tdmb_bb_sig_type *psigs)
{
  ...
  if(g_sync_status == RTV_TDMB_CHANNEL_LOCK_OK)// 채널 lock 일때만 값을 읽음
      psigs->PCBER = rtvTDMB_GetCER();//(int)RTV_TDMB_CER_DIVIDER;
  else
      psigs->PCBER = 20000;

  psigs->RSBER = rtvTDMB_GetPER();
  psigs->SNR = mtv350_Ant_Level(psigs->PCBER);//(rssi);//rtvTDMB_GetCNR() / (int)RTV_TDMB_CNR_DIVIDER;
  psigs->RSSI = abs(rssi);

  TDMB_MSG_BB("[%s] pcber[%d], rssi[%d] snr[%d] rsber[%d]\n", __func__, psigs->PCBER, /*psigs->RSSI*/rssi, psigs->SNR, psigs->RSBER);
...
void mtv350_test(int servicetype)
{
  ...
  res = mtv350_i2c_read(0x00); //0x8A 나와야 함
  ...
  stInfo->astSubChInfo[0].uiServiceType = (stInfo->astSubChInfo[0].uiServiceType==0x18) ? 0x02 : 0x01; //VIDEO 2, AUDIO 1
...
uint8 mtv350_i2c_test(void) 
{
  ...
  //mtv350_i2c_write(0x03, 0x87); // 32Mhz보다 클 경우 
  wData = mtv350_i2c_read(0x00); //0x8A 나와야 함
