...
#if  ((defined(RTV_TDMB_ENABLE) || defined(RTV_DAB_ENABLE)) && !defined(RTV_FIC_POLLING_MODE))
...
typedef struct
{
	...
	BOOL g_fTdmbPlayState;
#endif


static RTV_TDMB_REG_SUBCH_INFO g_atTdmbRegSubchInfo[RTV_MAX_NUM_SUB_CHANNEL_USED];
...
BOOL g_fRtvTdmbConfiguredFIC; //0928 build error
...
static void tdmb_InitCOMM(void)
{
	...
	RTV_REG_SET(0xA6, 0x30); // 0x30 ==>NO TSOUT@Error packet , 0x10 ==> NULL PID PACKET@Error packet
...
static void tdmb_InitOFDM(void)
{
	...
	switch( g_eRtvAdcClkFreqType )
	{
		...
		default:
			TDMB_MSG_RTV_BB("[tdmb_InitOFDM] Upsupport ADC clock type! \n");
			break;
	}
...
UINT rtvTDMB_GetLockStatus(void)
{
	...
	UINT lock_st = 0;
   	...
   	{
   		TDMB_MSG_RTV_BB("[_rtvTDMB_GetLockStatus] RTV Freqency change state! \n");	
		return 0x0;	 
	}
...
U32 rtvTDMB_GetPER(void)
{
	...
	if(g_fRtvChannelChange) 
	{
		TDMB_MSG_RTV_BB("[_rtvTDMB_GetPER] RTV Freqency change state! \n");
		return 0;	 
	}	
...
S32 rtvTDMB_GetRSSI(void) 
{
	...
	if(g_fRtvChannelChange) 
	{
		TDMB_MSG_RTV_BB("[_rtvTDMB_GetRSSI] RTV Freqency change state! \n");
		return 0;	 
	}	
...
U32 rtvTDMB_GetCNR(void)
{
	...
	if(g_fRtvChannelChange) 
	{
		TDMB_MSG_RTV_BB("[_rtvTDMB_GetCNR] RTV Freqency change state! \n");
		return 0;	 
	}	
...
U32 rtvTDMB_GetCER(void)
{
	...
	U32 cer_cnt, cer_period_cnt, val;

	if(g_fRtvChannelChange) 
	{
		TDMB_MSG_RTV_BB("[rtvTDMB_GetBER] RTV Freqency change state! \n");
		return 0;	 
	}	
	...
	cer_cnt = (rcnt3 << 24) | (rcnt2 << 16) | (rcnt1 << 8) | rcnt0; //0 ~ 65579

	TDMB_MSG_RTV_BB("[rtvTDMB_GetCER] cer_cnt: %u, cer_period_cnt: %u, old cer: %u\n", cer_cnt, cer_period_cnt, ((val / 1000)*25));

	//return ((val / 1000)*25);
	//return val / 1000;
	return (((val / 1000)/cer_period_cnt )*25);
	//return ((cer_cnt * RTV_TDMB_CER_DIVIDER)/cer_period_cnt);
}
...
U32 rtvTDMB_GetBER(void)
{
	...
	if(g_fRtvChannelChange) 
	{
		TDMB_MSG_RTV_BB("[_rtvTDMB_GetBER] RTV Freqency change state! \n");
		return 0;	 
	}	
...
static void tdmb_CloseSubChannel(UINT nRegSubChArrayIdx)
{
	...
	g_aRegSubChIdBits[SUBCH_GRP_IDX(nSubChID)] &= ~(1 << MOD32(nSubChID)); 
}
...
static void tdmb_OpenSubChannel(UINT nSubChID, E_RTV_TDMB_SERVICE_TYPE eServiceType, UINT nThresholdSize)
{
	...
	if(eServiceType == RTV_TDMB_SERVICE_VIDEO)
  	...
  	rtv_SetupMemory_MSC1(RTV_TV_MODE_TDMB, nThresholdSize);	
  
#else 
	/* Multi sub channel enabled */	
	if((eServiceType == RTV_TDMB_SERVICE_VIDEO) || (eServiceType == RTV_TDMB_SERVICE_AUDIO))
	{
		nHwSubChIdx = 0;		
								    ...
								    E_RTV_TDMB_SERVICE_TYPE eServiceType, UINT nThresholdSize)
{
	...
	if(g_dwTdmbPrevChFreqKHz == dwChFreqKHz)
	{
		...
		if(g_aRegSubChIdBits[SUBCH_GRP_IDX(nSubChID)] & (1<<MOD32(nSubChID)))
		{
			RTV_GUARD_LOCK;
...
#else
  RTV_DBGMSG0("[rtvTDMB_ReadFIC] FIC read \n");

  //memcpy(pbBuf, fic_buf, 384);
  //return 384;
...
void rtvTDMB_OpenFIC(void)
{
	...
	g_bRtvIntrMaskRegL &= ~(FIC_E_INT);
		...
		RTV_REG_MAP_SEL(HOST_PAGE);
    		RTV_REG_SET(0x62, g_bRtvIntrMaskRegL); 
   	   #endif	 /* #ifndef RTV_TDMB_FIC_POLLING_MODE */
	
    #else /* CIF Mode */
	 if(g_fTdmbPlayState == FALSE) // I2C read mode
	 {
		 RTV_REG_MAP_SEL(DD_PAGE);
		 RTV_REG_SET(0x35, 0x01); // FIC Interrupt status clear. 
	 ...
	 else
	 {
		 ...
		 bCon47 |= ( (1<<2) | (1<<5)); /* CIF/FIC */		 
		 RTV_REG_SET(0x47, bCon47); // CIF/FIC enable 
	 }
    #endif /* #if RTV_CIF_MODE_ENABLED */
#endif	    
...
INT rtvTDMB_ScanFrequency(U32 dwChFreqKHz)
{
	...
	{			
		...
		{
			TDMB_MSG_RTV_BB("[rtvTDMB_ScanFrequency] Scan Timeout! \n");
			...
			if(g_eRtvCountryBandType == RTV_COUNTRY_BAND_KOREA)
			{
				if(DAB_Mode > 0)   // Tr_mode detection miss for T-DMB [Only Tr_Mode 1ÀÎ °æ¿ì¿¡¸¸ »ç¿ë°¡´ÉÇÑ Á¶°Ç] 
				{
					...
					fail = 0xE002;
						...
						TDMB_MSG_RTV_BB("[rtvTDMB_ScanFrequency_DAB Fail] %d\n", DAB_Mode);
					goto TDMB_SCAN_EXIT;	 //Auto-scan result return  
				}
			}
...
TDMB_SCAN_EXIT:
	...
	//TDMB_MSG_RTV_BB("[rtvTDMB_ScanFrequency] 0x%04X\n", fail);
...
INT rtvTDMB_Initialize(E_RTV_COUNTRY_BAND_TYPE eRtvCountryBandType)
{
...
#ifdef RTV_CIF_MODE_ENABLED
    TDMB_MSG_RTV_BB("[rtvTDMB_Initialize] CIF enabled\n");
#else
    TDMB_MSG_RTV_BB("[rtvTDMB_Initialize] SINGLE CHANNEL enabled\n");
#endif
	...
	g_fTdmbPlayState = FALSE; // Default I2C FIC read mode.
	rtvCIFDEC_Init();	
#endif	
...
void mtv350_isr_handler(void)
{
	...
	U8 msc_tsize[2+1];	
	UINT msc_total = 0; // reset   
	MTV_CIF_MSC_BUF_INFO *cif_msc_buf = NULL;
#endif

	RTV_GUARD_LOCK;

   //TDMB_MSG_RTV_BB("[ISR Handler %d]\n", isr_handler++);
	RTV_REG_MAP_SEL(DD_PAGE);
	int_type_val1 = RTV_REG_GET(INT_E_STATL);
	//TDMB_MSG_RTV_BB("[mtv isr] 0x%02X\n", int_type_val1);

	/*==============================================================================
	 * Processing FIC and FEC RE_CONFIG interrupts.
	 * T-DMB/DAB: 1 FIC data
	 *============================================================================*/  
#if (defined(RTV_TDMB_ENABLE) || defined(RTV_DAB_ENABLE)) && !defined(RTV_FIC_POLLING_MODE) 	
	if(int_type_val1 & FIC_E_INT) // FIC interrupt
	{
		TDMB_MSG_RTV_BB("FIC_E_INT occured!\n");
		RTV_REG_MAP_SEL(FIC_PAGE);
...
#else
		/* DAB */
		if(mtv818_cb_ptr->fic_size != 0)
		{
	#if defined(RTV_IF_MPEG2_SERIAL_TSIF) || defined(RTV_IF_SPI_SLAVE) || defined(RTV_IF_QUALCOMM_TSIF) || defined(RTV_IF_MPEG2_PARALLEL_TSIF)
			RTV_REG_BURST_GET(0x10, fic_buf, mtv818_cb_ptr->fic_size/2);			  
			RTV_REG_BURST_GET(0x10, fic_buf+(mtv818_cb_ptr->fic_size/2), mtv818_cb_ptr->fic_size/2);	  
	#elif defined(RTV_IF_SPI) 
			RTV_REG_BURST_GET(0x10, fic_buf, mtv818_cb_ptr->fic_size+1);
	#endif	
		}
#endif
